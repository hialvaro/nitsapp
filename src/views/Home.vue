<template>
  <div v-if="dataLoaded" class="container mt-10 px-4">
    ...
    lg:grid-cols-4 gap-6 pb-10">
      <router-link :class="{'border-4 border-nits-light-green rounded-md': award.owned}" class="flex flex-col items-center bg-light-grey p-8 shadow-md cursor-pointer"
      ...
      <p :class="[{'bg-nits-blue text-white': award.type==='rar'},
                  {'bg-nits-green': award.type==='comú'},
                  {'bg-nits-violet': award.type==='èpic'},
                  {'bg-nits-gold': award.type==='llegendàri'}]" class="p-2 mt-6 py-1 text-xs text-white shadow-md rounded lg">
        {{award.type}}
      </p>

      <h1 v-if="award.owned" class="mt-8 mb-2 text-center text-xl text-nits-green">
        {{award.title}}
      </h1>
      ...
      <h1 v-if="award.howmany <= 30" class="p-2 mt-6 py-1 text-xs text-nits-green bg-grey shadow-md rounded lg">
        {{award.users.length}} / {{award.howmany}}
      </h1>

      <h1 v-if="award.howmany == 999" class="p-2 mt-6 py-1 text-xs text-nits-green bg-grey shadow-md rounded lg">
...
export default {
  ...
  setup() {
    ...
    return {data, dataLoaded, userLogged};
  },
};
</script>
