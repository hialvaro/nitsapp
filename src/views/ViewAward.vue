    ...
    <div v-if="dataLoaded">
      ...
      <div class="flex flex-col items-center p-8 rounded-md shadow-md bg-light-grey relative">
        <div v-if="user" class="flex absolute left-2 top-2 gap-x-2">
          <div v-if="data.owned" class="h-10 w-10 rounded-full flex justify-center items-center shadow-lg">
        ...
        <span v-if="data.owned" class="mt-6 py-1 5 px-5 text-xs text-white bg-nits-green rounded-lg shadow-md">
          {{data.type}}
        </span>
        <span v-else class="mt-6 py-1 5 px-5 text-xs text-white bg-dark-grey rounded-lg shadow-md">
          {{data.type}}
        </span>
      ...
      <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div class="flex flex-col items-center bg-nits-green shadow-md rounded-md cursor-pointer"
...
export default {
  ...
  setup() {
   ...
   const getData = async () => {
      try {
        const {data: awards, error} = await supabase.from('awards').select('*').eq('id', currentId);
        data.value = awards[0];
        ...
        if(user.value){
          ...
          for(let usr in data.value.users){
            profile = await supabase.from('profiles').select('*').eq('id', data.value.users[usr]);
    ...
    return { statusMsg, data, dataLoaded, errorMsg, user };
  },
};
</script>
